...
import { RPCConnectingCatchErrorException } from './exceptions';
...
export class Agent extends EventEmitter {
  ...
  private startPingPong() {
    this.timer = setInterval(() => {
      ...
      if (now - this.message.last_read_time > heatbeat || now - this.message.last_write_time > heatbeat) {
        ...
        const timer = setTimeout(() => this.destroy(), heatbeat * 3);
      }
    }, 1000);
  }
  ...
  private getConnection() {
    return new Promise<Websocket>((resolve, reject) => {
      ...
      const onerror = (e: any) => reject(new RPCConnectingCatchErrorException(e.message));
      ws.on('error', onerror);
      ws.on('close', () => {
        this.message.disable();
        this.socket = null;
