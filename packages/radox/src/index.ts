...
import { ClassMetaCreator, MethodMetaCreator } from '@typeservice/decorator';
import { parse, UrlWithParsedQuery } from 'url';
import { diff } from './utils';
import { EventEmitter } from 'events';
import { balances, TBalance } from './balance';
import { ServiceNameSpace, PublicNameSpace } from './decorators';
import { Exception } from '@typeservice/exception';
import { RPCInterfaceNotFoundException, RPCArgumentsEmptyException, RPCMethodNotFoundException, RPCAgentNotFoundException } from './exceptions';
...
export class Radox extends EventEmitter {
  ...
  private readonly balances = Object.assign({}, balances);
  ...
  constructor(props: RadoxProps) {
    ...
    this.server.on('connection', (socket, req) => {
      const _socket = new Socket(socket, this);
      _socket.on('close', () => {
  ...
  public addBalance(name: string, fn: TBalance) {
    this.balances[name] = fn;
    return this;
  }
  ...
  public async execute(command: string, value: any[]): Promise<any> {
    if (!value.length) throw new RPCArgumentsEmptyException();
    if (!this.namespaces.has(command)) throw new RPCInterfaceNotFoundException(command);
    const namespace = this.namespaces.get(command);
    const [method, ...args] = value;
    if (!namespace.methods.includes(method)) throw new RPCMethodNotFoundException(command, method);
  ...
  private async invoke(
    ...
    const getBalance = this.balances[balance || this.balance || 'default'];
    const agent = await Promise.resolve(getBalance(agents, method, args));
    ...
    const next = async (_agent: Agent) => {
      ...
      if (!_agents.length) throw new RPCAgentNotFoundException(command, method, args);
    ...
    if (!agent) {
      throw new RPCAgentNotFoundException(command, method, args);
    }
